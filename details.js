...
async function getAnimeDetails(malId) {
  try {
    const response = await fetch(
      `https://api.tenrai.org/v1/anime/${malId}/full`,
    );
    ...
    const airing =
      anime.status === "Currently Airing" && anime.broadcast.string
        ? `Airs on ${anime.broadcast.string}`
        : "";
    ...
    const animeRating =
      animeScore && animeScoredBy
        ? `<span class="badge border border-secondary fs-6 text-bg-dark px-2"><i class="bi bi-star-fill text-warning"></i> ${animeScore} <span style="font-size:14px">(${animeScoredBy})</span></span>`
        : "";
    const animeSeason =
      anime.season && anime.year ? `${anime.season} ${anime.year}` : "";
    const animeGenre = anime.genres
      .map((genre) => {
        return `<span class="badge border border-secondary">${genre.name}</span>`;
      })
      .join("");
    const animeStudio = anime.studios
      .map((studio) => {
    ...
    document.querySelector("#info-duration").textContent =
      anime.duration || "-";
    document.querySelector("#info-rank").textContent = anime.rank
      ? `#${anime.rank}`
      : "-";
    document.querySelector("#info-popularity").textContent = anime.popularity
      ? `#${anime.popularity}`
      : "-";
    document.querySelector("#info-members").textContent = anime.members
      ? anime.members.toLocaleString()
      : "-";

    // Add genre badges to info card
    const genreContainer = document.querySelector("#info-genre");
    genreContainer.innerHTML = anime.genres
      .map((genre) => {
    ...
    studioContainer.innerHTML = anime.studios
      .map((studio) => {
    ...
    if (anime.streaming && anime.streaming.length > 0) {
      platformsList.innerHTML = anime.streaming
        .map((stream) => {
      ...
      platformsList.innerHTML =
        '<span class="text-secondary small">Not available</span>';
    }
            ...
            requestQueue.add(async () => {
              try {
                const imageRes = await fetch(
                  `https://api.tenrai.org/v1/anime/${entry.mal_id}`,
                );
                ...
                if (imageData.status === 429) {
                  ...
                  const imageWrapper = document.querySelector(
                    `#relation-${entry.mal_id} .relation-image-wrapper`,
                  );
                  ...
                  const imageUrl =
                    imageData.data?.images?.webp?.image_url || "";
                  const imageWrapper = document.querySelector(
                    `#relation-${entry.mal_id} .relation-image-wrapper`,
                  );
          ...
          requestQueue.add(async () => {
            try {
              ...
              if (imageData.status === 429) {
                ...
                const imageWrapper = document.querySelector(
                  `#similar-${anime.mal_id} .similar-image-wrapper`,
                );
                if (imageWrapper && imageUrl) {
    ...
    try {
      let reviewsRes = await fetch(
        `https://api.tenrai.org/v1/anime/${malId}/reviews`,
      );
        ...
        if (reviewsData.data && reviewsData.data.length > 0) {
          ...
          const renderReviewItem = (rev, index) => {
            ...
            const toggleButtonHTML = showToggle
              ? `<button class="review-toggle-btn" id="review-btn-${index}">Read More</button>`
              : "";
